...
class ABP(object):
	...
	def propertyOnColvarsOutput(self): 
	...
	def appliedBiasPotential(self, coord_x, coord_y, d=None): 
	...
	def appliedBiasForce(self, coord_x, coord_y, d=None):# propagate the biasing potential 
		if self.abfCheckFlag == "yes" and self.nnCheckFlag == "yes": 
			if self.ndims == 1:
	...
	def getGlobalPartitionFunc(self): 
		if self.ndims == 1:
	...
	def getCurrentFreeEnergy(self): 
		...
		self.colvars_FreeE[np.isinf(self.colvars_FreeE)] = 0.0  # deal = inf
	...
	def learningProxy(self): 
		if self.nnCheckFlag == "yes":
			if self.frame % self.Frequency == 0 and self.frame != 0: 
				...
				self.instantOutput()
				self.biasingPotentialFromNN = -self.colvars_FreeE_NN # phi(x) = -Fhat(x) 
