...
class trainingNN(object):
	...
	def training(self, array_colvar_to_train, array_force_to_learn, learning_rate, regularFactor, epochs, outputFreq):
		...
		# layer 1
		node_12     = 96		
		w1          = tf.Variable(tf.truncated_normal([1, node_12], stddev=0.05)) #361*1 * 1*64 = 361*64
		b1          = tf.Variable(tf.zeros([self.size, node_12]))
		y1          = tf.nn.relu(tf.matmul(x, w1) + b1)
			
		# layer 2
		node_23     = 48	
		w2          = tf.Variable(tf.truncated_normal([node_12, node_23], stddev=0.05)) #361*64 * (64*48)=361*48
...
if __name__ == "__main__":
	...
	with open("Force_m1_T4_yesABF_yesNN_TL_2500.dat", "r") as fin:
		for line in fin:
	...
	force = output.training(array_colvar_to_train, array_force_to_learn, learning_rate=0.025, regularFactor=0, epochs=2800, outputFreq=100)
