...
if __name__ == "__main__":
	...
	output = trainingANN("loss.dat", "hyperparam.dat", 1, 361)
	array_target_to_learn = []

	with open("../../../estimateU", "r") as fin:
	#with open("estimate", "r") as fin:
		for line in fin:
			array_target_to_learn.append(line.split()[1])
	array_target_to_learn = np.array(array_target_to_learn)
	array_colvar_to_train = np.linspace(-np.pi, np.pi, 361)
	array_colvar_valid = np.linspace(-np.pi, np.pi, 722)
	#force = output.training(array_colvar_to_train, array_target_to_learn, learningRate=0.00845, regularFactor=0, epochs=10000, outputFreq=100) #adam
	#force = output.training(array_colvar_to_train, array_target_to_learn, learningRate=0.0800, regularFactor=0, epochs=2000, outputFreq=100)
	#forceNN, forceValid = output.training(array_colvar_to_train, array_target_to_learn, learningRate=0.0600, regularFactor=0.0, epochs=3500, outputFreq=100)
	#forceNN = output.training(array_colvar_to_train, array_target_to_learn, learningRate=float(sys.argv[1]), regularFactor=0.00027, epochs=3000, outputFreq=100)
	#forceNN, forceValid = output.training(array_colvar_to_train, array_target_to_learn, learningRate=float(sys.argv[1]), regularFactor=0.025, epochs=10000, outputFreq=100)
	forceNN, abpForce = output.training(array_colvar_to_train, array_target_to_learn, learningRate=float(sys.argv[1]), regularFactor=0.0000, epochs=3000, outputFreq=100)
	with open("out", "w") as fout:
		for cv, force in zip(array_colvar_to_train, forceNN):
			fout.write(str(cv) +  " " + str(force) + "\n")

	with open("out2", "w") as fout:
		for cv, abpforce in zip(array_colvar_to_train, abpForce):
			fout.write(str(cv) +  " " + str(-abpforce) + "\n")
