...
class trainingANN(object):
  ...
  def training(self, array_colvar_to_train, array_target_to_learn, learningRate, regularFactor, epochs, outputFreq):
    ...
    if self.ndims == 2: 
      ...
      layer1, w1, b1        = self.addDenseLayer(1, 70, tf.nn.sigmoid, None, CV_x, CV_y)
      layer2, w2, b2        = self.addDenseLayer(70, 50, tf.nn.sigmoid, None, layer1)
      layer3, w3, b3        = self.addDenseLayer(50, 35, tf.nn.sigmoid, None, layer2)
      layerOutput, w4, b4   = self.addDenseLayer(35, 1, None, "annOutput", layer3) #1681*1
