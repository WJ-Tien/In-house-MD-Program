...
class importanceSampling(object):
	...
	def LangevinEngine(self):
		...
		if self.abfCheckFlag == "yes":
			self.force_storage[int(np.floor(self.current_coord / self.binw)) + self.binNum//2] += self.sysForce(self.current_coord) 
			self.colvars_count[int(np.floor(self.current_coord / self.binw)) + self.binNum//2] += 1
			current_force = 0. # biased force applied
		else:
			current_force = self.sysForce(self.current_coord)
		...
		if self.abfCheckFlag == "yes":
			...
			next_force = self.sysForce(next_coord)
...
if __name__ == "__main__":
	...
	s = importanceSampling(0., 0., 0.005, 10000, 0, 1., 6.283185307179586, 4, 1., "yes", "LangevinEngine", "wABF001.dat", "wABF_force001.dat").mdrun()
